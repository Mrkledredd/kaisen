...
public class MenuStateUpdateMessage {
	...
	public static void handler(MenuStateUpdateMessage message, Supplier<NetworkEvent.Context> contextSupplier) {
		...
		context.enqueueWork(() -> {
			if (context.getSender().containerMenu instanceof KaisenModMenus.MenuAccessor menu) {
				menu.getMenuState().put(message.elementType + ":" + message.name, message.elementState);
				if (!context.getDirection().getReceptionSide().isServer() && Minecraft.getInstance().screen instanceof KaisenModScreens.ScreenAccessor accessor) {
	...
	public static void registerMessage(FMLCommonSetupEvent event) {
		KaisenMod.addNetworkMessage(MenuStateUpdateMessage.class, MenuStateUpdateMessage::buffer, MenuStateUpdateMessage::new, MenuStateUpdateMessage::handler);
	}
}